...
flowchart TB
    ...
    DB --> T_USERS & T_GS & T_GOALS & T_CYCLES & T_CYCLE_WINDOWS & T_TA & T_CHECKINS & T_NOTIF & T_AUDIT & T_ESC_RULES & T_ESC_LOGS & T_PWD
```
...
erDiagram
    ...
    goal_cycles ||--o{ goal_cycle_windows : "cycle has windows"
...
| `GET` | `/cycles` | ✅ | Any | List all goal cycles with per-phase windows + computed current phase |
...
- Goal setting window must be open (checked against `goal_cycle_windows` for `GOAL_SETTING`)
